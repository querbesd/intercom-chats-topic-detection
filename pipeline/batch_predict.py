...
class Predictor:
    ...
    #     # First pattern match attempt

    #     match = re.search("^https://[^/]+/v\\d+/(.+)$", request_url)
    #     if match:
    #         return match.group(1)
    #     else:
    #         # Second pattern match attempt for Azure OpenAI deployment URLs
    #         match = re.search(r"^https://[^/]+/openai/deployments/[^/]+/(.+?)(\?|$)", request_url)
    #         if match:
    #             return match.group(1)

    #     # If no match is found, handle the NoneType scenario
    #     raise ValueError(f"URL pattern not recognized: {request_url}")
